...
local function callSingletonFunc(singleton, name, ...)
    local args = {...}
    local singletonRef, typedef = table.unpack(singleton)
    return sdk.call_native_func(singletonRef, typedef, name, table.unpack(args))
end
...
local function checkIfInMultiplayer()
...
local function printDebugInfo()
    ...
    local a, b = pcall(function() 
        ...
        imgui.text("");
        imgui.text("Current quest type: " .. questTypeEnumMap[currentQuestType] ..
                       " (" .. currentQuestType .. ")")
        imgui.text("Current quest status: " ..
                       questStatusEnumMap[currentQuestStatus] .. " (" ..
                       currentQuestStatus .. ")")
        imgui.text(
            "Current fight music type: " .. fightBgmEnumMap[currentMusicType] ..
                " (" .. currentMusicType .. ")")
        imgui.text("Current music mix: " .. mixEnumMap[currentMixUsed] .. " (" ..
                       currentMixUsed .. ")")
...
local function getCurrentPlayer()
    ...
    local inGameInputDevice = getSingletonField(inputManager,
                                                "_InGameInputDevice")
    local playerInput = sdk.get_native_field(inGameInputDevice, getType(
                                                 "snow.StmInputManager.InGameInputDevice"),
                                             "_pl_input")
    local player = sdk.get_native_field(playerInput,
                                        getType("snow.StmPlayerInput"),
                                        "RefPlayer")
    return player
end
